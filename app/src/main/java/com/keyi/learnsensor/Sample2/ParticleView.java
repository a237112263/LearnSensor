...
public class ParticleView extends SurfaceView implements SurfaceHolder.Callback{
    public static final int DIE_OUT_LINE = 1000;//粒子的Y坐标超过该值会从粒子集合移除
    DrawThread dt;		//后台刷新屏幕线程
    ParticleSet ps;		//ParticleSet对象引用
    ParticleThread pt;	//ParticleThread对象引用
    String fps = "FPS:N/A";		//声明帧速率字符串
    //构造器，初始化主要成员变量
    public ParticleView(Context context, int width, int height) {
        ...
        pt = new ParticleThread(this,width,height);			//创建ParticleThread对象
    }
    //方法：绘制屏幕
